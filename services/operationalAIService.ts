  ...
  if (weeklyPlan && weeklyPlan.length > 0) {
    ...
    if (todayPlan) {
      ...
      taskList.forEach((taskItem: string | { text: string }, index: number) => {
        ...
        // Log AI action
        logAIAction('Task Generated', `Generated task: ${cleanTask} for ${assignedTo} at ${getOptimalTime(cleanTask, constraints.timePerDay)}`);
      });
    }
  }
  ...
  for (const alert of alerts) {
    if (!alert.sent) {
      ...
      if (alert.alertType === 'exhausted') {
        ...
        // Log AI action
        logAIAction('Stock Alert - Exhausted', `${alert.productName} is out of stock. Advertising tasks paused.`);
        ...
        // Log AI action
        logAIAction('Stock Alert - Critical', `${alert.productName} has only ${alert.currentStock} units left.`);
        ...
        // Log AI action
        logAIAction('Stock Alert - Low', `${alert.productName} is running low (${alert.currentStock} units).`);
...
export async function monitorSalesAndAlert(ownerMobile?: string): Promise<Notification | null> {
  ...
  // Log AI action
  logAIAction('Sales Monitoring', `Checking sales: Current ₹${currentRevenue.toLocaleString('en-IN')}, Previous ₹${previousRevenue.toLocaleString('en-IN')}`);
  ...
  if (previousRevenue > 0) {
    ...
    if (dropPercentage > 20) {
      ...
      const notification = await sendNotification(
        ...
        'high',
        '📉 Sales Drop Alert',
        `Sales have dropped by ${dropPercentage.toFixed(1)}% compared to last month. Current: ₹${currentRevenue.toLocaleString('en-IN')}, Previous: ₹${previousRevenue.toLocaleString('en-IN')}. Review your marketing strategy.`,
        ownerNumber || undefined
      );
...
export async function checkFestivalsAndRemind(ownerMobile?: string): Promise<Notification[]> {
  ...
  for (const occasion of occasions) {
    ...
    if (daysUntil === 3) {
      ...
      // Log AI action
      logAIAction('Festival Reminder', `${occasion.name} is in ${daysUntil} days. Reminder sent for website and Instagram advertising.`);

      // Send promotional alert to customers
      await sendCustomerNotification(
        'festival',
        `${occasion.name} Special Offers!`,
        `🎉 ${occasion.name} is coming! Visit us for special promotions and exclusive offers. Stay tuned for updates!`
      );
    }
    ...
    if (daysUntil === 1) {
      ...
      // Log AI action
      logAIAction('Festival Reminder - Urgent', `${occasion.name} is TOMORROW! Urgent reminder sent.`);
    }
  }
...
// Log AI actions
function logAIAction(action: string, description: string) {
