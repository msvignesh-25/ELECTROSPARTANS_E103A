...
export default function EnhancedSchedulingCalendar({
  ...
  const checkAllAlerts = async () => {
    ...
    const todayOccasions = occasions.filter(occ => {
      ...
      return occDate.toDateString() === new Date().toDateString();
    });

    // Send reminders for today's occasions
    todayOccasions.forEach(occasion => {
      if (!occasion.advertisingReminders.webpage || !occasion.advertisingReminders.instagram) {
            ...
            {specialOccasions.map((occasion) => {
              ...
              return (
                <li key={occasion.id} className="text-sm text-purple-800 dark:text-purple-200">
                  <span className="font-semibold">{occasion.name}</span> - {isToday ? 'TODAY!' : `in ${daysUntil} day${daysUntil !== 1 ? 's' : ''}`}
                  {isToday && (
                    <div className="mt-1 text-xs">
                      ...
                      {ownerMobile && <span className="text-green-600 dark:text-green-400">✓ WhatsApp reminder sent</span>}
                    </div>
                  )}
                </li>
              );
